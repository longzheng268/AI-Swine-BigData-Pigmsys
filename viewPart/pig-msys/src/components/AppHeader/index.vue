<template>
  <div class="header">
    ...
    <el-dialog title="修改密码" v-model="dialogFormVisible" width="400px">
      <el-form :model="ruleForm" status-icon :rules="rules" ref="ruleForm" label-width="100px" style="margin-top: -40px;width: 300px;margin-bottom: -20px" size="mini">
    ...
    return{
      ...
      rules:{
        ...
        newpass:[
          ...
          { min: 6, max: 32, message: '密码必须为长度在6到32个字符', trigger: ['blur','change'] }
        ],
        checkpass:[
  ...
  methods: {
    submitForm(formName) {
      ...
      formRef.validate((valid) => {
        if (valid) {
          pwdApi.updatePwd(this.user.id,this.ruleForm.newpass).then(response=>{
            const resp=response.data
            ElMessage({
