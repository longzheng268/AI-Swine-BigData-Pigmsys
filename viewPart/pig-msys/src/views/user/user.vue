  ...
  <div>
    <el-form ref="searchForm" :inline="true" :model="searchWhere" class="demo-form-inline" style="margin-top: 20px">
      <el-form-item label="姓名" size="small" prop="name">
        <el-input v-model="searchWhere.name" placeholder="姓名" style="width: 150px"></el-input>
      </el-form-item>
      <el-form-item label="家庭地址" size="small" prop="address">
        <el-input  v-model="searchWhere.address" placeholder="家庭地址" style="width: 150px"></el-input>
      </el-form-item>
      <el-form-item label="角色" size="small" prop="userRole">
        <el-select v-model="searchWhere.userRole" placeholder="用户等级" style="width: 120px">
          <el-option v-for="option in userRoleOptions" :label="option.name" :value="option.role" :key="option.role"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item label="性别" size="small" prop="sex">
        <el-select v-model="searchWhere.sex" placeholder="用户性别" style="width: 120px">
          <el-option v-for="option in userSexOptions" :label="option.name" :value="option.sex" :key="option.sex"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item size="small">
        <el-button type="primary" plain @click="searchUserList" icon="el-icon-search">查询</el-button>
        <el-button type="success" plain class="el-icon-refresh" @click="resetForm('searchForm')">重置</el-button>
        <el-button type="warning" plain class="el-icon-plus" @click="HandlerAdd('searchData')">新增</el-button>
      </el-form-item>
    </el-form>
    <el-table ref="searchData"
        :data="userInfoList"
        style="width: 100%"
        max-height="500">
      <el-table-column
      ...
      <el-form :model="form" :rules="rules" label-width="100px" label-position="right" ref="addForm" style="width: 400px;margin-top: -20px">
        ...
        <el-form-item label="角色" size="small" prop="userrole">
          <el-select v-model="form.userrole">
            <el-option :label="option.name" :value="option.role" v-for="option in userRoleOptions" :key="option.role"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
      <template #footer>
...
export default {
  ...
  methods:{
    searchUserList(){
      ...
      userinfoApi.search(this.currentPage,this.pageSize,this.searchWhere).then(response=>{
        const resp=response.data
        if(resp.flag){
