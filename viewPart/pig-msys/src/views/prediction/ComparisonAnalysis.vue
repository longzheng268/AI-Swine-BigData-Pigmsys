<template>
  <div class="comparison-container">
    <h2>生长预测方案对比</h2>
    <p class="description">输入两组不同的饲养参数，直观对比不同品种或饲养方式下的预期生长效果</p>
        ...
        <el-card shadow="hover">
          ...
          <el-form :model="plan1" label-width="100px" size="default">
            <el-form-item label="方案名称">
              <el-input v-model="plan1.name" placeholder="例如：白猪标准方案"></el-input>
            </el-form-item>
            <el-form-item label="日龄">
              <el-input-number v-model="plan1.age" :min="1" :max="365" style="width: 100%"></el-input-number>
              <span class="unit">天</span>
            </el-form-item>
            <el-form-item label="饲料摄入量">
              <el-input-number v-model="plan1.feed" :min="0" :max="10" :precision="2" style="width: 100%"></el-input-number>
      ...
      <el-col :xs="24" :sm="12" :md="12">
        <el-card shadow="hover">
          ...
          <el-form :model="plan2" label-width="100px" size="default">
            <el-form-item label="方案名称">
              <el-input v-model="plan2.name" placeholder="例如：黑猪优化方案"></el-input>
            </el-form-item>
            <el-form-item label="日龄">
              <el-input-number v-model="plan2.age" :min="1" :max="365" style="width: 100%"></el-input-number>
              <span class="unit">天</span>
            </el-form-item>
            <el-form-item label="饲料摄入量">
              <el-input-number v-model="plan2.feed" :min="0" :max="10" :precision="2" style="width: 100%"></el-input-number>
              <span class="unit">kg/天</span>
            </el-form-item>
            <el-form-item label="品种">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            ...
            <el-table :data="comparisonTable" stripe border style="width: 100%">
              <el-table-column prop="metric" label="指标" width="180" align="center"></el-table-column>
              <el-table-column prop="plan1" :label="plan1.name || '方案一'" align="center">
                <template #default="scope">
                  <span :class="{'better-value': scope.row.better === 1}">{{ scope.row.plan1 }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="plan2" :label="plan2.name || '方案二'" align="center">
                <template #default="scope">
                  <span :class="{'better-value': scope.row.better === 2}">{{ scope.row.plan2 }}</span>
                </template>
              </el-table-column>
              <el-table-column prop="difference" label="差异" align="center">
                <template #default="scope">
                  <el-tag v-if="scope.row.difference" :type="scope.row.diffType">
            ...
            <el-alert
              ...
              <template #default>
                <ul style="margin: 10px 0; padding-left: 20px;">
                  <li v-for="(suggestion, index) in analysisSuggestions" :key="index">
                    {{ suggestion }}
                  </li>
                </ul>
              </template>
  ...
  methods: {
      ...
      this.comparisonTable = [
        {
          ...
          diffType: weight2 > weight1 ? 'success' : (weight2 < weight1 ? 'danger' : 'info'),
        ...
        },
        {
          ...
          diffType: rate2 > rate1 ? 'success' : (rate2 < rate1 ? 'danger' : 'info'),
        ...
        },
        {
          ...
          diffType: confidence2 > confidence1 ? 'success' : (confidence2 < confidence1 ? 'danger' : 'info'),
          better: confidence2 > confidence1 ? 2 : (confidence2 < confidence1 ? 1 : 0)
        },
        {
          metric: '品种',
        ...
        },
        {
          ...
          difference: this.plan2.feed !== this.plan1.feed ? 
    ...
    generateAnalysis(result1, result2) {
      ...
      if (this.analysisSuggestions.length === 0) {
        this.analysisSuggestions.push('两个方案各有优势，建议根据实际成本和市场需求选择合适的方案')
      }
    },
