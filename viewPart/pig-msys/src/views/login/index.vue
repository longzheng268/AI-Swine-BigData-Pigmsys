  ...
  <div id="login-container">
    <el-form ref="form" :rules="rules" :model="form" label-width="60px" class="login-form">
      ...
      <el-form-item label="验证码" prop="code" label-width="65px">
          ...
          <div class="verify">
            <img :src="imgUrl" alt="更换验证码" @click="getVerify(this)" width="100px" />
          </div>
        </el-form-item>
      <el-form-item>
        <el-button type="primary" @click="submitForm('form')" style="width: 100px">登录</el-button>
        <el-button type="primary" @click="resetForm('form')" style="width: 100px; margin-left: 35px">重置</el-button>
      </el-form-item>
      <el-form-item class="register-button">
        <el-button link @click="showRegisterDialog" style="width: 100%; margin-top: 5px">注册新账号</el-button>
      </el-form-item>
    </el-form>
    ...
    <el-dialog title="用户注册" v-model="registerDialogVisible" width="500px">
      <el-form :model="registerForm" :rules="registerRules" ref="registerForm" label-width="100px">
        <el-form-item label="账号" prop="username">
          <el-input v-model="registerForm.username" placeholder="请输入账号（4-20个字符）"></el-input>
        </el-form-item>
        <el-form-item label="密码" prop="userpassword">
          <el-input v-model="registerForm.userpassword" placeholder="请输入密码（6-18个字符）" type="password" show-password></el-input>
        </el-form-item>
        <el-form-item label="确认密码" prop="confirmPassword">
          <el-input v-model="registerForm.confirmPassword" placeholder="请再次输入密码" type="password" show-password></el-input>
        </el-form-item>
        <el-form-item label="手机号" prop="usermobile">
          <el-input v-model="registerForm.usermobile" placeholder="请输入手机号"></el-input>
        </el-form-item>
        <el-form-item label="角色" prop="roleId">
    ...
    return {
      ...
      rules:{
        ...
        userpassword: [
          ...
          { min: 6, max: 18, message: '密码长度必须为6到18个字符', trigger: ['blur','change']}
        ],
        roleId: [
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {//表单验证成功，开始向后端传送数据进行验证
          aVerify(this.form.code).then(resp=>{
            if (resp.data.flag){
              login(this.form.username,this.form.userpassword)
                .then(resp=>{
                  ElMessage({
                    message: '登录成功',
                    type: 'success',
                  ...
                  if(respLogin.flag){
                    getUserInfo(respLogin.data.token)
                      .then(resp=>{
                        /*this.$message({
                          message: '获取用户信息成功',
                          type: 'success'
                        ...
                        if(respUserInfo.flag){
                          ...
                          localStorage.setItem("my-login-user",JSON.stringify(userInfo))
                          localStorage.setItem("my-login-token",respLogin.data.token)
    ...
    handleRegister() {
      this.$refs.registerForm.validate((valid) => {
        if (valid) {
          ...
          // 调用注册接口
          myaxios.post('/user/register', registerData)
            .then(response => {
