  ...
  <div class="visualization-container">
    ...
    <el-card style="margin-top: 20px;">
      ...
      <el-table :data="monitorPoints" border>
        <el-table-column prop="monitorPoint" label="监测点" width="150"></el-table-column>
        <el-table-column prop="monitorLocation" label="位置" show-overflow-tooltip></el-table-column>
        ...
        <el-table-column label="状态" width="80">
          <template #default="scope">
            <el-tag :type="scope.row.isAbnormal === 1 ? 'danger' : 'success'" size="small">
...
export default {
  ...
  methods: {
    initCharts() {
      try {
        // 检查 DOM 元素是否存在
        if (!this.$refs.temperatureChart || !this.$refs.humidityChart || 
            !this.$refs.co2Chart || !this.$refs.nh3Chart || 
            !this.$refs.radarChart || !this.$refs.pieChart) {
    ...
    renderCharts() {
      ...
      if (!this.temperatureChart || !this.humidityChart || 
          !this.co2Chart || !this.nh3Chart || 
          !this.radarChart || !this.pieChart) {
    ...
    renderTemperatureChart() {
      ...
      const xData = this.chartData.map(item => item.collectTime?.substring(5, 16) || '')
      ...
      const option = {
        ...
        series: [{
          ...
          markLine: {
            data: [
              { yAxis: 18, label: { formatter: '最低标准 18℃' }, lineStyle: { color: '#E6A23C' } },
              { yAxis: 28, label: { formatter: '最高标准 28℃' }, lineStyle: { color: '#E6A23C' } }
            ]
          }
        }]
      }
    ...
    renderCo2Chart() {
      ...
      const xData = this.chartData.slice(0, 10).map(item => item.monitorPoint || '')
      const yData = this.chartData.slice(0, 10).map(item => item.co2Concentration || 0)
    ...
    renderNh3Chart() {
      ...
      const xData = this.chartData.slice(0, 10).map(item => item.monitorPoint || '')
      const yData = this.chartData.slice(0, 10).map(item => item.nh3Concentration || 0)
    ...
    renderPieChart() {
      ...
      const normalCount = this.chartData.filter(item => item.isAbnormal === 0).length
      const abnormalCount = this.chartData.filter(item => item.isAbnormal === 1).length
      ...
      const option = {
        ...
        series: [{
          ...
          radius: '50%',
          data: [
            { value: normalCount, name: '正常数据', itemStyle: { color: '#67C23A' } },
            { value: abnormalCount, name: '异常数据', itemStyle: { color: '#F56C6C' } }
          ],
          emphasis: {
            itemStyle: {
              shadowBlur: 10,
    ...
    calculateAverage() {
      let tempOk = 0, humOk = 0, co2Ok = 0, nh3Ok = 0, lightOk = 0
