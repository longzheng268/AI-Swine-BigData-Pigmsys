  ...
  <div class="environment-container">
    ...
    <el-card class="search-card">
      <el-form :inline="true" :model="searchForm">
        <el-form-item label="监测点">
          <el-input v-model="searchForm.monitorPoint" placeholder="请输入监测点名称" clearable></el-input>
        </el-form-item>
    ...
    <el-card style="margin-top: 20px;">
      <el-table :data="tableData" border stripe style="width: 100%">
        ...
        <el-table-column prop="monitorPoint" label="监测点" width="120"></el-table-column>
        ...
        <el-table-column prop="co2Concentration" label="CO₂(ppm)" width="100"></el-table-column>
        <el-table-column prop="nh3Concentration" label="氨气(ppm)" width="100"></el-table-column>
        <el-table-column prop="collectTime" label="采集时间" width="160"></el-table-column>
        ...
        <el-table-column prop="abnormalReason" label="异常原因" show-overflow-tooltip></el-table-column>
    ...
    <el-dialog :title="dialogTitle" v-model="dialogVisible" width="600px">
      <el-form :model="formData" :rules="formRules" ref="dataForm" label-width="120px">
        ...
        <el-form-item label="温度(℃)" prop="temperature">
          <el-input-number v-model="formData.temperature" :precision="2" :min="0" :max="50"></el-input-number>
        </el-form-item>
        <el-form-item label="湿度(%)" prop="humidity">
          <el-input-number v-model="formData.humidity" :precision="2" :min="0" :max="100"></el-input-number>
        </el-form-item>
        <el-form-item label="CO₂浓度(ppm)" prop="co2Concentration">
          <el-input-number v-model="formData.co2Concentration" :precision="2" :min="0"></el-input-number>
        </el-form-item>
        <el-form-item label="氨气浓度(ppm)" prop="nh3Concentration">
          <el-input-number v-model="formData.nh3Concentration" :precision="2" :min="0"></el-input-number>
        </el-form-item>
        <el-form-item label="光照强度(lux)" prop="lightIntensity">
          <el-input-number v-model="formData.lightIntensity" :precision="2" :min="0"></el-input-number>
        </el-form-item>
        <el-form-item label="采集时间" prop="collectTime">
          <el-date-picker v-model="formData.collectTime" type="datetime" placeholder="选择日期时间"></el-date-picker>
        </el-form-item>
      </el-form>
      <template #footer>
        <span>
...
<script>
import { searchEnvironmentData, addEnvironmentData, deleteEnvironmentData } from '@/api/environment'
  ...
  methods: {
    loadData() {
      const params = {
        ...
        isAbnormal: this.searchForm.isAbnormal === '' ? undefined : this.searchForm.isAbnormal
      }
      
      searchEnvironmentData(this.currentPage, this.pageSize, params).then(response => {
    ...
    calculateStats() {
      this.abnormalCount = this.tableData.filter(item => item.isAbnormal === 1).length
      const uniquePoints = new Set(this.tableData.map(item => item.monitorPoint))
      this.monitorPointCount = uniquePoints.size
      this.normalRate = this.totalCount > 0 ? ((this.totalCount - this.abnormalCount) / this.totalCount * 100).toFixed(1) : 0
    },
    handleSearch() {
      this.currentPage = 1
      this.loadData()
      ...
      this.$refs.dataForm.validate((valid) => {
        ...
        if (valid) {
          ...
          const submitData = {
            ...
            latitude: this.formData.latitude ? Number(this.formData.latitude) : null,  // 确保是数字
            ...
            co2Concentration: this.formData.co2Concentration ? Number(this.formData.co2Concentration) : null,
            nh3Concentration: this.formData.nh3Concentration ? Number(this.formData.nh3Concentration) : null,
            lightIntensity: this.formData.lightIntensity ? Number(this.formData.lightIntensity) : null,
            isAbnormal: 0,  // 默认正常
            abnormalReason: null
          }
          ...
          if (this.formData.collectTime) {
            ...
            submitData.collectTime = d.getFullYear() + '-' + 
            ...
            // 如果没有指定时间，使用当前时间
            const now = new Date()
            submitData.collectTime = now.getFullYear() + '-' + 
          ...
          addEnvironmentData(submitData).then(response => {
            ...
            if (error.response) {
              ...
              if (error.response.status === 403) {
                ...
                ElMessage.error('添加失败：' + (errorData?.message || error.message || '未知错误'))
              }
            } else {
